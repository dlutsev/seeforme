...
func (s *Userservice) GetStatistics(ctx context.Context) (int64, int64, error) {
	volunteersCount, blindCount, err := s.db.GetUsersCount(ctx)
	if err != nil {
		s.log.Error("failed to get users count", "error", err)
		return 0, 0, ErrGetUser
	}
